...
$column = "{$table}.*, {$table_branch}.branch_name";

$whereClause = '1'; //永遠條件成立
$orderby = "{$table_branch}.branch_name ASC,{$table}.room_sort ASC,{$table}.updated_date ASC";
// like search 對應db格式
$searchColumn = [
...
$sql="SELECT {$column} FROM {$table} 
	  LEFT JOIN {$table_branch} ON {$table}.branch_id = {$table_branch}.branch_id
	  where {$whereClause} 
	  ORDER BY {$orderby} 
	  LIMIT {$start},{$limit}";
